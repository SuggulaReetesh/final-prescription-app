...
import 'package:prashantclinic/main.dart';
...
class _addclinicalState extends State<addclinical> {
  ...
  CollectionReference ref =
      FirebaseFirestore.instance.collection('patientdetails1');
  CollectionReference regids =
      FirebaseFirestore.instance.collection('RegistrationId1');
        ...
        actions: [
          TextButton(
              ...
              onPressed: () {
                ...
                widget.user['receipts'] = receipts;
                //update this one

                ref.add(widget.user).whenComplete(() {
                  regids.doc('user').update({'id': (widget.user['regno'] + 1)});
                  showDialog(
                      context: context,
                      builder: (context) {
                        return AlertDialog(
  ...
  Future<void> _createPDF() async {
    ...
    page.graphics.drawString(widget.user['regno'].toString(),
    ...
    page.graphics.drawString(widget.user['patientname']+' ' +widget.user['patientmiddlename']+' '+widget.user['patientlastname'],
        PdfStandardFont(PdfFontFamily.helvetica, 16),
    ...
    page.graphics.drawString(widget.user['consultationby'],
        PdfStandardFont(PdfFontFamily.helvetica, 16),
    ...
    page.graphics.drawString(
        widget.user['age'], PdfStandardFont(PdfFontFamily.helvetica, 16),
    ...
    page.graphics.drawString(getgender(widget.user['gender']),
        PdfStandardFont(PdfFontFamily.helvetica, 16),
    ...
    page.graphics.drawString('Spo2: ' + widget.user['spo2'],
        PdfStandardFont(PdfFontFamily.helvetica, 16),
        bounds: Rect.fromLTWH(0, 220, page.getClientSize().width, 50));
    page.graphics.drawString('PR    : ' + widget.user['p'],
        PdfStandardFont(PdfFontFamily.helvetica, 16),
        bounds: Rect.fromLTWH(0, 240, page.getClientSize().width, 50));
    page.graphics.drawString('BP    : ' + widget.user['BP'],
        PdfStandardFont(PdfFontFamily.helvetica, 16),
        bounds: Rect.fromLTWH(0, 260, page.getClientSize().width, 50));
    page.graphics.drawString('Wt     : ' + widget.user['Weight'],
        PdfStandardFont(PdfFontFamily.helvetica, 16),
        bounds: Rect.fromLTWH(0, 280, page.getClientSize().width, 50));
    if (rtpcrcheck == true) {
      page.graphics.drawString('Rt-pcr: ' + widget.user['rtpcr'],
    ...
    document.dispose();
    saveAndLaunchFile(bytes, widget.user['patientname'] + '.pdf');
  }
  ...
  getregno() async {
    await FirebaseFirestore.instance
        .collection('RegistrationId1')
        .get()
        .then((QuerySnapshot querySnapshot) {
      querySnapshot.docs.forEach((doc) {
        widget.user['regno'] = doc['id'];
        print(doc['id']);
      });
      widget.user['appointdate'] =
